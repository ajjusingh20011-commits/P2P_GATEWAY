...
function accentHex(accent) {
  if (!accent) return ACCENT_HEX.purple;
  return accent[0] === '#' ? accent : ACCENT_HEX[accent] || ACCENT_HEX.purple;
}
...
export function StatCard({ label, value, sub, icon: Icon, accent = 'indigo', trend, index = 0 }) {
    ...
    <Card style={{ padding: '20px 22px' }}>
      <div className="flex items-start justify-between">
        <div className="min-w-0">
          ...
          {trend && (
            <p style={{ marginTop: 6, fontSize: 11, fontWeight: 600, color: trend.up ? '#22c55e' : '#ef4444' }}>
    ...
    <div className="fixed inset-0 z-50 flex items-start justify-center overflow-y-auto bg-black/60 p-4 backdrop-blur-sm">
      ...
      >
        <div className="flex items-start justify-between p-5" style={{ borderBottom: '1px solid var(--cardborder)' }}>
          ...
          <button onClick={onClose} aria-label="Close" className="tf-hbtn">
...
/** Small copy-to-clipboard button with transient "Copied" feedback handled by caller. */
export function CopyButton({ value, label = 'Copy', className = '' }) {
  return (
    <button
      onClick={() => navigator.clipboard?.writeText(value)}
      className={`inline-flex items-center gap-1 rounded-lg px-2.5 py-1.5 text-xs font-medium ${className}`}
      style={{ background: 'var(--hover)', border: '1px solid var(--cardborder)', color: 'var(--text)' }}
    >
      {label}
    </button>
  );
