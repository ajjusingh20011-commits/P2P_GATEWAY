...
const SEEN_KEY = 'admin-notif-last-seen-id';

// adminApi.listOrders() returns the already-unwrapped { orders, pagination }.
...
export default function NotificationBell() {
  ...
  const load = async () => {
    ...
    setError(false);
    try {
      const data = await adminApi.listOrders({ limit: 20 });
      setItems(extractOrders(data));
    } catch (e) {
      setError(true);
    } finally {
      setLoading(false);
          ...
          {!loading && !error && items.map((o) => (
            <div key={o.id} style={{ padding: '11px 14px', borderBottom: '1px solid var(--cardborder)' }}>
              <div className="flex items-center justify-between gap-3">
                ...
                <span style={{ color: '#14b8c4', fontSize: 13, fontWeight: 700, whiteSpace: 'nowrap' }}>₹{o.amount_inr}</span>
              </div>
              <div className="flex items-center gap-2" style={{ marginTop: 3 }}>
                ...
                {o.merchant?.business_name && (
                  <span style={{ color: 'var(--muted)', fontSize: 11, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap', maxWidth: 150 }}>
                    {o.merchant.business_name}
                  </span>
                )}
