...
const NAV = [
  ...
  { to: '/orders', label: 'Orders', icon: IconOrders, badge: 'orders' },
  { to: '/payments', label: 'Payments', icon: IconPayments },
  { to: '/payouts', label: 'Payouts', icon: IconPayouts, badge: 'payouts' },
      ...
      {collapsed ? (
        <div className="flex flex-col items-center gap-2" style={{ padding: '0 0 12px' }}>
          <span
            ...
            style={{ width: 36, height: 36, borderRadius: 11, background: '#ef4444', fontSize: 13 }}
          >
            P2P
          </span>
        ...
        <div className="flex items-center justify-between" style={{ padding: '4px 6px 14px' }}>
          <div className="flex items-center gap-2.5">
            ...
            <div style={{ lineHeight: 1.25 }}>
              <div style={{ color: 'var(--text)', fontWeight: 700, fontSize: 15 }}>Admin Console</div>
              <div style={{ color: '#ef4444', fontSize: 10, fontWeight: 600, letterSpacing: '.5px' }}>CONTROL CENTER</div>
            </div>
          </div>
          ...
          return (
            <NavLink key={to} to={to} className={linkClass} data-tip={collapsed ? label : undefined} style={collapsedNavStyle}>
              <span style={{ position: 'relative', display: 'flex' }}>
                ...
                {collapsed && hasCount && (
                  <span
                    style={{ position: 'absolute', top: -3, right: -4, width: 8, height: 8, borderRadius: '50%', background: '#ef4444', border: '1.5px solid var(--sidebar)' }}
                  />
                )}
              </span>
              {!collapsed && (
